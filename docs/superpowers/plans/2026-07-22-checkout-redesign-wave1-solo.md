...
## Execution Addendum (2026-07-22, post-audit — plan was written against a stale branch)

Audit of worktree (origin/main-based) vs plan assumptions. **This section overrides the tasks above where they conflict.**

- **Task 1: DONE** (commits d53f562e + 07551b28). Main already had `modeHint` handling both `?mode=team|individual`; net delta = division links + tests.
- **Task 2: SKIPPED — superseded.** Main already guards deposit ≥ price in `src/lib/registrations/amount-due.ts:19` (silent fallback to full price), `registration-wizard.tsx:143` `depositValid()`, and `payment-step.tsx:156-161` `depositAvailable`. Do not add `deposit-policy.ts`; do not change fallback semantics to a throw.
- **Task 3: as written.** Single call site is `registration-wizard.tsx:515`. Hardcode `variant: "v1"` there until Task 5.
- **Task 4: as written.** Next migration is `0103_*`. Null-audit must include `src/lib/registrations/resolve-person.ts` (types `birthDate: string` at lines 23/32/62/80/93; dependent-dedupe `eq()` predicate — dependent path keeps requiring DOB, self path becomes nullable).
- **Task 5 adjustments:**
  - The guest player-step gate is `computeGuestErrors()` (wizard line ~1114) called from `handleContinue()` (~1140) with `guestAttempted`/`guestFieldErrors` — put the v2 short-circuit there (v2 requires only first/last/email); `canProceed()` is secondary.
  - `handleSubmitGuestCheckout` (~851) guards `!waiverAccepted || !waiverSignature` on line 1 and hardcodes `waiverSigned: true` in both payload branches — restructure: v2 skips the guard and sends `waiverSigned: false`, no `birthDate`.
  - `GuestInfoStep`: add `minimal?: boolean`; thread alongside existing `lockedMode`.
  - P5: only `enterKeyHint` + `autoCapitalize` are missing (autocomplete/inputMode already exist).
  - E2E: `tests/e2e/registration-adult.spec.ts` AND `tests/e2e/registration-adult-guest.spec.ts` run the v1 agreements flow against `e2e-adult-open-soccer-2026` (minAge 18 → flips to v2). Both must be updated to the v2 flow. Seed already has the adult open season (`ADULT_OPEN_SEASON_SLUG = "e2e-adult-open-soccer-2026"`, seed line 181) — do NOT add another.
  - Test convention: no `tests/api/helpers/` module exists; follow the existing per-file pattern (`const ADULT_OPEN_SEASON_SLUG = "e2e-adult-open-soccer-2026"` + DB lookup by slug, as in `tests/api/registrations-self.test.ts:10`).
- **Task 6 adjustments:** as written, PLUS include `registrationId: regResult.registration.id` in the clientSecret response branch (guest-checkout.ts ~line 371) — Task 8 needs it on the confirm screen; today only waitlisted/paid_zero paths return it.
- **Task 7 adjustments:** drop the seeded `UNSIGNED_REG_ID` fixture. The API test mints its own: POST guest-checkout v2 with a unique email (new user → response carries the session Set-Cookie), then POST `/api/registrations/{id}/complete` with that cookie. Payment status is irrelevant to signing. Registration id comes from the Task 6 response addition.
- **Task 8 adjustments:** two precursor steps: (a) extract the waiver legal text from `waiver-step.tsx:81-111` into a shared `src/components/registration/waiver-text.tsx` and use it in both waiver-step and CompletionForm — never fork the text; (b) `confirmation-step.tsx` is 41 lines with props `{ seasonName, registrantDisplayName, isSelf? }` — thread `registrationId` + `waiverSigned` from the wizard's checkout/confirm state into it. `/account` middleware gate exists (`middleware.ts:76`).
- **Task 9 adjustments:** email CTA links must work for a signed-out recipient: build them with `createMagicLink`/`buildMagicLinkUrl` using an existing purpose + destination `/account/complete/{registrationId}?via=email_link`, following how `send-balance-reminders.ts` builds its links (same purpose it uses, unless destination validation in `magic-link-destination.ts` forbids that path — only then add a purpose, and if `MagicLinkPurpose` is a pg enum, the enum-add must be its OWN migration, per repo rule). `email_logs.emailType` is varchar(50) — reminder type strings need no schema change. Confirmation-email send site: `src/lib/stripe/handle-registration-payment-succeeded.ts:178`.
- **Task 10 adjustments:** endpoint `src/pages/api/admin/registrations.ts` already selects `waiverSigned` (line 69); `registrations-list.tsx` shows it as text in the expanded panel (line 335). Deliverable narrows to: inline badges on collapsed rows + "Needs attention" filter + `ageReviewNeeded` in endpoint select + badge.
