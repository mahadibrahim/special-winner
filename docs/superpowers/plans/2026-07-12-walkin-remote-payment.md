...
- NO SQL backfill EVER (superseding the earlier follow-up-PR idea): any migration statement that USES the new enum value breaks CI's from-empty single-transaction migration run permanently (all files pending together). Instead, Task 3's sweep gains a LEGACY branch: rows with status pending_claim AND promotionExpiresAt IS NULL AND createdAt < now - 2h are expired exactly like payment holds (cancelled, reason expired_payment_hold, waitlist promoted). All prod stranded holds are >2h old at deploy, so the first cron tick clears them; new walk-in holds are pending_payment from Task 2 onward, so the branch goes quiet but stays as a guard.
- Task 2 addition (T1 review finding): the one-active-per-user-session partial unique index predicate does NOT cover pending_payment and cannot be extended by migration (same enum-use-in-transaction hazard). Task 2 MUST add an application-level duplicate guard in walkin/start: inside the insert transaction, reject (409) when the user already has a confirmed/waitlisted/pending_claim/pending_payment booking on the session. The index-predicate extension is deferred to a future migration-runner improvement (per-file transactions) — record in the PR body.
...
## Task 5 scope amendment (Task 2 inventory, category D)

Task 5 additionally updates every surface where `pending_claim` meant "active hold" (full classified inventory in .superpowers/sdd/payment-task-2-report.md):
- Duplicate-booking guards: `src/lib/dropin/guest-checkout.ts`, `src/pages/api/dropin/sessions/[id].ts`, `BookButton.tsx` — active-statuses lists gain `pending_payment` (without this a customer can double-book a session they already hold).
- Admin session detail: `/admin/dropin/sessions/[id]` + `AdminSessionDetail.tsx` — pending_payment rows render like holds.
- Parent dashboard: `normalize-bookings.ts`, `MyDropInBookings.tsx`, `PlayAttention.tsx`, `PlayUpcoming.tsx` — pending_payment renders as a "complete payment" pending state (reuse the pending_claim treatment; link target is the self-serve token where surfaced).
Interim breakage note: between Tasks 2 and 5 the branch intentionally fails 3 hold-visibility assertions + the held-walkin e2e — tracked, resolves in Task 5, nothing merges before Task 8.
