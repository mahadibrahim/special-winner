...
| `role.parent` | Customer | Youth program guardian | Family handbook |
| `role.player` | Customer | Adult registrant / athlete-self-account | Player handbook |
...
- The 11-role set is intentional. New roles require explicit catalog-level discussion, not per-activity decisions.
...
2. **Role view** — one chapter per role. Filter: `accountable = R` ∪ `responsible includes R`. Used in employee, contractor, and customer manuals (same generator, different role IDs).
...
|---|---|---|
| `expected - 15min` | Pre-reminder | Responsible |
| `expected + 15min` | Overdue alert | Responsible + Accountable |
| `expected + 60min` | Escalation | `escalation_path` target |

Activities can override per-record where the default doesn't fit.
...
### `pre_game` (T-2h to T-0, per match) — 12 activities
...
| `act.captain_pregame_briefing` | `event_lead` | `signature` | `manual` | format:league (adult — captain-led teams) |
| `act.coach_pregame_briefing` | `event_lead` | `signature` | `manual` | format:league (youth — coach-led teams) |
...
**Total: 61 activities across 7 phases.**

### Coverage validation

- **All 7 phases populated** ✓
- **All non-customer roles as `accountable` at least once:** venue_manager (16), event_lead (7), facilities (11), front_of_house (6), ref (4), photographer (3), platform (11), director (3). Sums to 61 ✓
- **Automation mix:** 11 platform, 18 hybrid, 32 manual — demonstrating realistic distribution where most activities still need humans ✓
...
- **Format-conditional activities:** 3 (league briefings ×2, drop-in walk-on ×1) ✓
...
**Signatures (7):** `sig.staff_briefing_signin`, `sig.ref_check_in`, `sig.photographer_check_in`, `sig.captain_pregame`, `sig.coach_pregame`, `sig.ref_score_attestation`, `sig.staff_clock_out`.
...
**Total: 61 artifact templates referenced.** Each requires a stub file in the catalog and an implementation ticket. Stubs without implementation render as "*(pending implementation)*" in role manuals until the platform feature ships.
...
1. **Reminder delivery channels.** Pre-reminders, overdue alerts, and escalations need channel selection: SMS (Twilio), Telegram (already wired), email (Resend), in-app push. Probably configurable per role (e.g., refs prefer SMS; venue managers prefer Telegram).
2. **Multi-event-per-day tracking.** A venue running 6 matches has 6 cycles of `pre_game/in_game/post_game` activities. The tracking engine needs to handle per-match instances of the same activity without confusion.
3. **Activity completion override.** What happens when an activity legitimately can't complete (a coach doesn't show, so coach pregame briefing can't happen)? Need a "skip with reason" path that's still tracked, vs simply going overdue.
4. **Catalog change → live-event impact.** If the catalog changes mid-season, do already-scheduled events use the old or new activity definitions? Probably old (snapshot at event creation), but worth confirming.
5. **Customer-role manuals (parent/player) — depth.** These have very few activities they're `responsible` for. Might not warrant a full generated manual; could be a single-page reference instead.
6. **Age/audience tagging.** Captain vs coach pregame briefings differ by adult vs youth, but `format_tags` doesn't currently distinguish. Options: (a) split format into `adult_league` / `youth_league` etc., (b) add a new `audience_tags` dimension, (c) leave as a per-activity description note. Decision deferred until a second adult/youth divergence appears in the catalog.
