...
- **Card-on-file is notice, not opt-in** (owner decision 2026-07-23): the consent checkbox is **removed**. The fee box states the terms as its final line — *"Your card stays on file for the team. Teammate shares still unpaid after {date} are charged to it. Your $200 counts toward the team fee."* — and the reserve button binds them ("By reserving, you agree to the payment terms above"), the standard place-order pattern. The API keeps recording `backstopConsentedAt` at submission (the client keeps sending `backstopConsent: true`); only the client-side checkbox gate goes away. The existing E2E assertion on checkbox gating is rewritten accordingly.
