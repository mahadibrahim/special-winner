...
| # | Item | Status | Owner | Notes |
|---|---|---|---|---|
| 10 | Audit: idempotency key coverage on every PaymentIntent.create site | ✓ | Eng | Full grep audit: 6 call sites total. 5 had idempotency keys; 1 (`src/pages/api/admin/dropin/sessions/[id]/walk-up.ts:167` — front-desk Terminal walk-up) was missing one and is a real double-charge risk on a fast double-tap. Fixed in this PR with key derived from `sessionId:dropin-walkup-pi:userId:amountCents`. |
...
| 25 | Venue-scoped rosters + reports pages | □ | Eng | Build `/admin/venue/rosters` (player roster filtered to the manager's locations) and `/admin/reports/venue` (revenue + registration counts filtered to the manager's locations). Both were in the venue-manager sidebar but routed to 404s; dropped from the nav in this PR. Add back once the pages exist. Estimate: 1-2 days each. Trigger to prioritize: first hired facility manager who needs them. |
| 26 | Venue-manager data scoping audit | ✓ | Eng | DONE (PR after #139). Approach was "lock-down + scope". Middleware `SUPER_ADMIN_ONLY_PREFIXES` extended to cover the URL-reachable pages a venue manager doesn't need (registrations, teams, games, payments, reports, announcements, broadcasts). New `requireSuperAdminAccess` helper added; 15 matching APIs now enforce super-only auth so a `location_admin`'s session token can't curl past the page lock. The three venue-visible APIs that don't get locked (lookup, waitlist — refund-requests was already scoped in #139) now narrow by `getLocationIdsForUser` for non-super-admins. Announcements dropped from venue-manager sidebar since the data model has no per-location field. **Residual gaps** captured as #28-30 below. |
| 28 | Per-location announcements | ✓ | Eng | DONE (PR after #142). Migration 0037 added `announcements.location_id` (nullable FK). NULL = org-wide (super-admin only); non-null = scoped to that venue (location_admin can create/edit/delete at their locations). `/admin/announcements` re-enabled for location_admin (removed from middleware lock list); added back to venue-manager sidebar. Editor gets a scope select that hides "Org-wide" for non-super; defaults to the picker's pinned venue when one is set. List shows a scope chip per row. Fan-out email recipient query narrowed for venue-scoped rows (only families with a registration at that location get the email). `/api/user/announcements` filtered so families only see org-wide + announcements at locations they have registrations at. |
| 29 | `/api/admin/games` location scope | ✓ | Eng | DONE (PR after #140). Resolved by the realization that `/admin/media/shoots/bulk` is super-admin-by-intent (other media users access the customer-facing `/media` surface, not `/admin/media`). With `/admin/media` locked to super in #30, the only caller of `/api/admin/games` was already a super-admin, so the endpoint flipped to `requireSuperAdminAccess` directly — no "super OR media" gate needed. |
| 30 | `/admin/game-day`, `/admin/activity-completions`, `/admin/media` lockdown | ✓ | Eng | DONE (PR after #140). All three added to middleware `SUPER_ADMIN_ONLY_PREFIXES`. Coaches access curriculum tracking via `/coach/*` (separate route + API surface), media users access via the customer-facing `/media`, so the `/admin/*` equivalents are super-admin-by-intent oversight. 13 backing APIs (games + 6 activity-completions + 6 media) flipped to `requireSuperAdminAccess` in the same pass. |
| 27 | Venue picker for multi-venue admins | ✓ | Eng | DONE (PR after #141). Cookie-backed (`aspire_active_venue`), validated in middleware against caller's scope, exposed via `Astro.locals.activeLocationId`. Self-fetching picker component (`VenuePicker`) mounted in `AdminLayout` reads its own state from `GET /api/admin/active-venue` so the .astro pages don't have to thread two new props through 50+ admin routes. `getEffectiveLocationIds()` helper narrows queries to the pinned venue (or full scope if unpinned). Wired into the four already-scoped surfaces: waitlist API GET, lookup API GET, refund-requests page, check-in venues list. Mutations (POST/PUT) deliberately keep using `getLocationIdsForUser` directly — picker is a view filter, not a permission gate. Picker auto-hides for callers with <2 pickable venues. |
