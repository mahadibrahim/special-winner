...
| 28 | Per-location announcements | □ | Eng | Announcements have no `locationId` — they're org-scoped only. The temporary fix in #26 was to lock the entire announcements surface to super-admin. The real fix is a schema addition (`announcements.locationId` nullable, with `NULL` meaning org-wide) plus a UI toggle on the editor and a location filter on the listing. Estimate: ~1 day incl. migration. Trigger: facility manager needs to send announcements without going through super-admin. |
