...
| 26 | Venue-manager data scoping audit | □ | Eng | Most `/admin/**` pages and APIs filter by `orgId` only, not by `getLocationIdsForUser`. For Aspire's two-facility org (Downtown + Worthington), this means a Downtown manager sees Worthington data on announcements, lookup, waitlist, check-in venue list, registrations, payments, etc. Helpers exist (`getLocationIdsForUser`, `requireSameLocation`) — work is mechanical: thread them through each listing query for non-super-admins. Estimate: 1-2 days. Trigger to prioritize: before hiring any non-super-admin facility staff. |
