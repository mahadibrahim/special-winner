...
| 2 | Dispute / chargeback flow | □ | Eng | **Confirmed real gap.** No `charge.dispute.*` handler. No admin UI. No schema. Audit estimate: 1-2 days for minimal (webhook handler + email alert to founder) or 3-5 days for full (handler + admin disputes list + evidence upload). Recommend minimal scope first — most disputes need fast human attention, not in-app workflows. |
| 3 | Webhook delivery monitoring | □ | Founder + Eng | **Confirmed real gap.** Failures hit `console.error` + PostHog `captureException` (webhook handler only). No durable alerting (no Slack/email/PagerDuty). Cheapest path: (a) enable Stripe dashboard email alerts on webhook failures — zero code, founder action; (b) wire a PostHog action/webhook to ping founder when a `captureException` event lands. Total: ~4-6 hours work + dashboard config. |
...
| 11 | PostHog `captureException` coverage outside the webhook | □ | Eng | Audit confirms only the Stripe webhook calls `captureException`. Cron jobs, API routes, payment helpers do not. Scope: add `captureException` in the cron job catch blocks and in the registration/payment helpers' top-level catch. ~2-3 hours. |
