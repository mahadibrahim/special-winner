...
| 29 | `/api/admin/games` location scope | □ | Eng | Skipped from the #26 lockdown because the media bulk-shoot page (`/admin/media/shoots/bulk`) calls it and the media surface has its own auth model (`media_staff`/`media_editor`). Real fix is either (a) add an explicit "super or media" gate, or (b) decompose into a games-list endpoint for super-admin + a separate read-only endpoint for media scheduling. Estimate: ~0.5 day. Trigger: token-leak risk taken seriously, or media flow needs to scope its games view. |
| 30 | `/admin/game-day`, `/admin/activity-completions`, `/admin/media` lockdown | □ | Eng | Three additional URL-reachable admin surfaces that a venue manager can hit. Game-day and activity-completions are coach/super tools; media has the `media_*` auth model. None were in #26's lockdown to avoid bleeding scope into the coach + media access models. Estimate: ~0.5 day per surface once we decide each surface's intended audience. Trigger: same as #28 — first non-super, non-coach, non-media facility staff. |
| 27 | Venue picker for multi-venue admins | □ | Eng | Admins scoped to multiple locations (org-scoped, or multi-location-scoped) have no UI to pick which venue they're operating on. The sidebar subtitle just says "N venues". Add a picker that scopes downstream pages. Estimate: 1 day. Trigger: when a director-tier role lands or when an admin manages both Downtown + Worthington. |
