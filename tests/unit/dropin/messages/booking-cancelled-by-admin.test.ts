...
describe("renderBookingCancelledByAdmin", () => {
  it("uses session-cancelled subject + apology copy when reason is session_cancelled", () => {
    const v = renderBookingCancelledByAdmin(baseCtx);
  ...
  it("uses booking-cancelled subject when reason is admin_refund", () => {
    const v = renderBookingCancelledByAdmin({
      ...baseCtx,
      reason: "admin_refund",
    });
  ...
  it("notes the refund amount when refunded is true and amount > 0", () => {
    const v = renderBookingCancelledByAdmin(baseCtx);
  ...
  it("notes 'no charge' when amountPaidCents is 0", () => {
    const v = renderBookingCancelledByAdmin({
      ...baseCtx,
