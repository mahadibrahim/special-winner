...
describe("endEnrollmentsForMembership (the subscription-deleted cascade)", () => {
  // The `customer.subscription.deleted` webhook can't be driven from here
  // (it needs STRIPE_CONNECT_WEBHOOK_SECRET and the SoccerOne fixture org —
  // see tests/api/memberships/webhook-subscription-lifecycle.test.ts, which
  // skips wholesale without them). The handler is a two-liner whose delegation
  // is unit-tested; the REAL SQL — the part that regressed — is this library
  // function, exercised here against the same staging DB every other test in
  // this file uses.

  it("ends the membership's enrollments AND releases their future $0 seats, sparing paid ones", async () => {
    const db = getDb();
    const suffix = Date.now();
    const templateId = await createTemplate(`Delete-Churn-${suffix}`, 5);
    const childId = await createTestChild(parentUserId, `Churner-${suffix}`);
    const membershipId = await createTestChildMembership({
      userId: parentUserId,
      familyMemberId: childId,
      organizationId,
      tierId,
      idSuffix: `churner-${suffix}`,
    });
    ...
    // Ending the standing seat alone used to leave this booking standing: the
    // cron had already materialized it, so a churned family kept a confirmed
    // class they no longer pay for and the seat stayed off the waitlist.
    const futureSessionId = await createClassSession(hoursFromNow(72), templateId);
    const [booking] = await db
      .insert(dropInBookings)
      .values({
    ...
    // A PAID make-up survives a churn for the same reason it survives a quit:
    // this path can't refund it, so it isn't this path's to cancel.
    const paidSessionId = await createClassSession(hoursFromNow(96), templateId);
    const [paidBooking] = await db
      ...
      .values({
        ...
        stripePaymentIntentId: `pi_test_churn_${suffix}`,
      })
      .returning({ id: dropInBookings.id });

    // A waitlisted sibling on the freed session — post-commit promotion runs
    // in the webhook path too.
    const waitlistChildId = await createTestChild(parentUserId, `ChurnWaiter-${suffix}`);
    const [waitlisted] = await db
      .insert(dropInBookings)
      .values({
        sessionId: futureSessionId,
    ...
    expect(await endEnrollmentsForMembership(membershipId)).toEqual({ endedCount: 1 });

    const [enrollmentAfter] = await db
      .select({ status: classEnrollments.status, endedAt: classEnrollments.endedAt })
      .from(classEnrollments)
      .where(eq(classEnrollments.id, enrollmentId));
    expect(enrollmentAfter.status).toBe("ended");
    expect(enrollmentAfter.endedAt).not.toBeNull();
    ...
    expect(released.cancellationReason).toBe("user_request");
    ...
    // Replay-safe: a redelivered webhook ends nothing and releases nothing.
    expect(await endEnrollmentsForMembership(membershipId)).toEqual({ endedCount: 0 });
  });
});
