...
const ENDPOINT = "/api/cron/send-balance-reminders"

describe("Cron: send balance reminders", () => {
  ...
  it("authenticated POST returns per-window summary with all three windows", async () => {
    ...
    expect(types).toEqual(["t1", "t21", "t7"])
    for (const w of json.windows) {
      ...
      expect(typeof w.daysBefore).toBe("number")
    }
  })
  ...
  it("second authenticated run is idempotent — sent counts do not double", async () => {
    ...
    // that the second never exceeds the first plus the window count, and is
    // 0 when nothing remains. We don't assert the first count.
