/**
 * GET /api/kiosk/[venueSlug]/search
 *
 * Seeds a drop-in session TODAY at the E2E rental venue and a confirmed
...
describe("GET /api/kiosk/[venueSlug]/search", () => {
  ...
  it("returns the booking when searching by partial first name", async () => {
    ...
    const res = await apiFetch(
      `/api/kiosk/${E2E_RENTAL_VENUE_ID}/search?q=${encodeURIComponent(q)}`,
    );
  ...
  it("returns the booking when searching by last-4 of phone", async () => {
    ...
    const res = await apiFetch(
      `/api/kiosk/${E2E_RENTAL_VENUE_ID}/search?q=${encodeURIComponent(last4)}`,
    );
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(Array.isArray(body.results)).toBe(true);
  ...
  it("returns empty results for an empty query", async () => {
    const res = await apiFetch(`/api/kiosk/${E2E_RENTAL_VENUE_ID}/search?q=`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("returns empty results for a single-char query (below threshold)", async () => {
    const res = await apiFetch(`/api/kiosk/${E2E_RENTAL_VENUE_ID}/search?q=A`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.results).toEqual([]);
  });

  it("returns 404 for a non-UUID venueSlug", async () => {
