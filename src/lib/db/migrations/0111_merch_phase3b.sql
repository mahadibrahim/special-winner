-- Merch Phase 3b (rebased onto main's 0110). Idempotent + state-agnostic:
-- the kit-dependent backfill is wrapped in a `merch_team_kits` existence guard,
-- so this migration is CORRECT on a fresh DB (kit table/column present, from
-- 0109) and a clean NO-OP on an already-migrated DB (kit table/column already
-- dropped). PL/pgSQL never plans an un-taken IF branch, so referencing the
-- since-dropped kit_id/merch_team_kits inside the guard is safe. Enum ADD VALUEs
-- are safe in-tx here (nothing in this migration uses the new values).
...
-- kit-dependent backfill, guarded on merch_team_kits existence ---------------
DO $$
BEGIN
  IF to_regclass('public.merch_team_kits') IS NOT NULL THEN
    -- one general store per org that has non-kit products
    ...
    WHERE EXISTS (SELECT 1 FROM "merch_products" p WHERE p."organization_id" = o."id" AND p."kit_id" IS NULL)
      AND NOT EXISTS (SELECT 1 FROM "merch_stores" s WHERE s."organization_id" = o."id" AND s."scope" = 'general');
    UPDATE "merch_products" p SET "store_id" = s."id"
    FROM "merch_stores" s
    WHERE p."kit_id" IS NULL AND p."store_id" IS NULL AND s."organization_id" = p."organization_id" AND s."scope" = 'general';
    -- each existing kit -> an unlisted team store
    ...
    WHERE NOT EXISTS (SELECT 1 FROM "merch_stores" s WHERE s."share_token" = k."share_token");
    UPDATE "merch_products" p SET "store_id" = s."id"
    FROM "merch_team_kits" k
    JOIN "merch_stores" s ON s."share_token" = k."share_token" AND s."scope" = 'team'
    WHERE p."kit_id" = k."id" AND p."store_id" IS NULL;
  END IF;
END $$;--> statement-breakpoint

-- order provenance from each order's own items (always; no kit dependency) ---
...
-- slug uniqueness swap org -> store ----------------------------------------
...
DROP INDEX IF EXISTS "idx_merch_products_org_active";--> statement-breakpoint

-- drop kit remnants (after backfill) ---------------------------------------
