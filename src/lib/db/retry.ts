 ...
 * into ~30-minute windows during a Railway connectivity blip. Rarer sustained
 * degradations also happen (a ~43h `read ECONNRESET` window on
 * 2026-07-18/19): those outlast any retry budget that fits a Netlify function
 * timeout, so they correctly surface to telemetry — resolve the PostHog
 * issues once recovered rather than widening retries.
 *
 ...
 * real work. The pool defaults to `max: 5` (DB_POOL_MAX, see
 * src/lib/db/index.ts); the probe establishes the first connection so a job's
 * opening queries don't trip the connect timeout — further pool connections
 * only open lazily under concurrent query load. The probe is a plain
