...
const ORANGE = "var(--primary-bright)";
...
export function LandingTabs({ sport, basePath, overview, ruleSections, pastEmptyCopy, current, upcoming, past }: Props) {
  ...
  return (
    <div>
      <div className="bg-ink px-9">
        <div className="max-w-[1080px] mx-auto flex gap-0.5">
          {tabs.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)} aria-selected={tab === t.key}
              className={cn("text-xs font-bold tracking-[0.1em] uppercase px-4 py-3.5 cursor-pointer relative top-px flex items-center gap-1.5",
                tab === t.key ? "bg-sand text-ink" : "text-cream/70")}>
      ...
      <div className="bg-cream min-h-[360px]" data-testid="landing-tabs">
        <div hidden={tab !== "overview"}>
          <>
            <div className="band-ink px-9 py-9">
              <div className="max-w-[1080px] mx-auto">
                ...
                <div className="grid md:grid-cols-3 gap-3.5">
                  {overview.why.map((v) => (
                    <div key={v.title} className="bg-ink-lift border border-rule-on-ink p-4">
                      <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center text-lg mb-2.5", TINT[v.tint])}>{v.icon}</div>
                      <div className="font-display text-base mb-0.5">{v.title}</div>
                ...
                {current && (
                  <a href={`${basePath}/${current.slug}`} data-testid="overview-season-cta"
                    ...
                    <span className="flex items-center gap-4">
                      ...
                      <span><span className="font-display font-black text-2xl leading-none block">{current.label}</span><span className="font-mono text-xs text-ink/70 mt-1 block">{current.dateLine} · {current.divisions} divisions · {current.venues} venues</span></span>
                    </span>
            ...
            <div className="px-9 py-9"><div className="max-w-[1080px] mx-auto">
              <p className="label text-ink-muted mb-3">{overview.midLabel}</p>
                ...
                <div className="grid md:grid-cols-2 gap-3.5">
                  {overview.midSlot.items.map((d) => (
                    <div key={d.title} className="bg-paper border border-cream-3 rounded-xl p-4">
                      <div className="display-s mb-0.5" style={{ fontSize: "1.0625rem" }}>{d.title}</div>
                      <p className="text-[12.5px] text-ink-2 leading-snug">{d.copy}</p>
                    </div>
                  ))}
                </div>
              )}
              <p className="label text-ink-muted mt-9 mb-3">The rules, in brief</p>
              <div className="grid md:grid-cols-2 gap-x-7 gap-y-4">
                {ruleSections.map((s) => (<div key={s.title}><h3 className="font-display text-lg mb-1">{s.title}</h3><ul className="space-y-1">{s.items.slice(0, 3).map((it) => <li key={it} className="text-[12.5px] text-ink-2 leading-snug">· {it}</li>)}</ul></div>))}
              </div>
            </div></div>
          </>
        </div>
        <div hidden={tab !== "this"}>
          <div className="px-9 py-9"><div className="max-w-[1080px] mx-auto">
            <h2 className="font-display text-2xl mb-4">This season</h2>
            {current ? (
              <a href={`${basePath}/${current.slug}`}
                ...
                <span><span className="font-display font-black text-2xl block">{current.label}</span><span className="font-mono text-xs text-cream/80 mt-1 block">{current.dateLine} · {current.divisions} divisions · {current.venues} venues</span></span>
        ...
        <div hidden={tab !== "upcoming"}>
          <div className="px-9 py-9"><div className="max-w-[1080px] mx-auto">
            <h2 className="font-display text-2xl">Upcoming seasons</h2>
            ...
            {upcoming.length ? upcoming.map((t) => (
              <div key={t.slug} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 bg-paper border border-cream-3 rounded-xl px-5 py-4 mb-2.5">
                <span><span className="font-display font-black text-xl block">{t.label}</span><span className="font-mono text-xs text-ink-muted mt-1 block">{t.meta} · {t.opensLabel}</span></span>
                <a href={`${basePath}/${t.slug}`} className="text-[11px] font-extrabold tracking-[0.1em] uppercase border-[1.5px] border-ink text-ink px-4 py-2.5 whitespace-nowrap">Notify me →</a>
              </div>
        ...
        <div hidden={tab !== "past"}>
          <div className="px-9 py-9"><div className="max-w-[1080px] mx-auto">
            <h2 className="font-display text-2xl">Past seasons</h2>
            ...
            {past.length ? past.map((t) => (
              <a key={t.slug} href={`${basePath}/${t.slug}`} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 bg-paper border border-cream-3 rounded-xl px-5 py-4 mb-2.5">
                <span className="font-display font-black text-xl">{t.label}</span>
                <span className="label text-primary-text" style={{ fontSize: "0.6875rem" }}>Results →</span>
              </a>
