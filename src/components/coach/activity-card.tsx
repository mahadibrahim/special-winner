...
const TYPE_CONFIG: Record<string, { icon: typeof Target; color: string; bg: string }> = {
  ...
  technical: { icon: Target, color: "text-primary", bg: "bg-primary/10" },
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-ink-muted capitalize">{activity.activityType}</span>
                </div>
              </div>
            </div>
            {onAdd && (
        ...
        <div className="space-y-6 mt-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-lg bg-paper border border-border">
              ...
              <p className="text-sm font-medium text-ink">{activity.durationMinutes} min</p>
            </div>
            <div className="p-3 rounded-lg bg-paper border border-border">
              <div className="flex items-center gap-2 text-ink-muted mb-1">
            ...
            <div className="p-3 rounded-lg bg-paper border border-border">
              ...
              <p className="text-sm font-medium text-ink capitalize">{activity.spaceRequired || "Any"}</p>
            </div>
          </div>
          ...
          {activity.description && (
            <div>
              <h4 className="text-sm font-medium text-ink-muted mb-2">Overview</h4>
              <p className="text-sm text-ink-2">{activity.description}</p>
            </div>
          )}
          ...
          {activity.setupInstructions && (
            <div>
              <h4 className="text-sm font-medium text-ink-muted mb-2">Setup</h4>
              <p className="text-sm text-ink-2 bg-paper p-3 rounded-lg border border-border">
                {activity.setupInstructions}
              </p>
            </div>
          )}
          ...
          {/* How to Play */}
          <div>
            <h4 className="text-sm font-medium text-ink-muted mb-2">How to Play</h4>
            <div className="text-sm text-ink-2 bg-paper p-3 rounded-lg border border-border whitespace-pre-line">
              {activity.howToPlay}
            </div>
          </div>
          ...
          {activity.coachingPoints && activity.coachingPoints.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-ink-muted mb-2 flex items-center gap-2">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {activity.makeEasier && (
              <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                ...
                <p className="text-sm text-ink-2">{activity.makeEasier}</p>
              </div>
            )}
            {activity.makeHarder && (
              <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20">
                ...
                <p className="text-sm text-ink-2">{activity.makeHarder}</p>
              </div>
            )}
          </div>
          ...
          {activity.variations && activity.variations.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-ink-muted mb-2">Variations</h4>
              <div className="space-y-2">
                {activity.variations.map((variation, i) => (
                  <div key={i} className="p-3 rounded-lg bg-paper border border-border">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-medium text-ink">{variation.name}</span>
                      <Badge variant="outline" className="text-[10px] capitalize">
                        {variation.difficulty}
                      </Badge>
                    </div>
                    <p className="text-sm text-ink-muted">{variation.description}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activity.equipmentNeeded && activity.equipmentNeeded.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-ink-muted mb-2">Equipment Needed</h4>
              ...
              <div className="flex flex-wrap gap-1.5">
                {activity.tags.map((tag, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className="text-[10px] bg-paper border-border text-ink-muted"
                  >
                    #{tag}
                  </Badge>
                ))}
      ...
      <>
        <div
          className={cn(
            ...
            "bg-paper border-border hover:border-border hover:bg-cream-2",
            className
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h4 className="text-sm font-medium text-ink truncate">{activity.name}</h4>
    ...
    <>
      ...
      >
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="font-medium text-ink truncate">{activity.name}</h3>
        ...
        {activity.description && (
          <p className="text-sm text-ink-muted line-clamp-2 mb-3">{activity.description}</p>
        )}
