...
const TYPE_CONFIG: Record<string, { icon: typeof Target; color: string; bg: string; label: string }> = {
  ...
  technical: { icon: Target, color: "text-blue-400", bg: "bg-blue-500/20", label: "Technical" },
...
function SegmentRow({
  ...
  if (isEditing) {
    return (
      <div className="p-4 rounded-xl border bg-white/[0.04] border-primary/30">
        <div className="space-y-3">
          ...
          <Textarea
            ...
            className="min-h-[60px] bg-white/[0.02] border-white/[0.06]"
          />
  ...
  return (
    <div className="group relative">
      {/* Timeline connector */}
      <div className="absolute left-6 top-12 bottom-0 w-0.5 bg-gradient-to-b from-white/20 to-transparent group-last:hidden" />

      <div className={cn(
        "relative p-4 rounded-xl border transition-all",
        "bg-white/[0.02] border-white/[0.06]",
        !readOnly && "hover:border-white/10"
      )}>
        {/* Timeline dot */}
        <div
          className={cn(
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                {segment.activityName && (
                  <Badge variant="outline" className="text-[10px] bg-white/[0.02] border-white/[0.06]">
                    {segment.activityName}
                  </Badge>
                )}
              </div>
              ...
              {segment.notes && (
                <p className="text-sm text-gray-400 mt-2 line-clamp-2">{segment.notes}</p>
              )}
            </div>
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-medium text-gray-400">Session Timeline</h3>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className={cn(
            "font-medium",
            totalDuration && currentTotalDuration > totalDuration ? "text-red-400" : "text-white"
          )}>
      ...
      {segments.length === 0 ? (
        <div className="text-center py-8 border border-dashed border-white/10 rounded-xl">
          ...
          <p className="text-xs text-gray-600 mt-1">Add segments to build your session</p>
        </div>
      ) : (
        ...
        <>
          {showAddForm ? (
            <div className="p-4 rounded-xl border bg-white/[0.02] border-white/[0.06]">
              <h4 className="text-sm font-medium text-white mb-3">Add Segment</h4>
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  <Input
                  ...
                  <select
                    ...
                    className="h-9 px-3 rounded-md bg-white/[0.02] border border-white/[0.06] text-sm text-white"
                  >
            ...
            <Button
              variant="outline"
              className="w-full border-dashed border-white/10 hover:border-white/20 hover:bg-white/[0.02]"
