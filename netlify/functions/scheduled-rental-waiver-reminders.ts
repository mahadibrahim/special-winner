/**
 * Netlify Scheduled Function — triggers the field-rental unsigned-player
 * waiver reminder sweep twice daily by POSTing to the
 * /api/cron/rental-waiver-reminders route.
 *
 ...
 * surface failures without retrying (the next tick is up to 12 hours away).
 */
...
export const handler = schedule("0 9,17 * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-rental-waiver-reminders] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-rental-waiver-reminders] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-rental-waiver-reminders]", err);
    return {
      statusCode: 500,
