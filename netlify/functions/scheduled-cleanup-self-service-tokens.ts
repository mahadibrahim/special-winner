/**
 * Netlify Scheduled Function — triggers the self-service tokens expiry
 * cleanup sweep every day at midnight UTC by POSTing to the
 * /api/cron/cleanup-self-service-tokens route.
 *
...
const ROUTE = "/api/cron/cleanup-self-service-tokens";

export const handler = schedule("41 0 * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-cleanup-self-service-tokens] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-cleanup-self-service-tokens] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-cleanup-self-service-tokens]", err);
    return {
      statusCode: 500,
