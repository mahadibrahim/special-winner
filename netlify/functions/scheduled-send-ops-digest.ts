/**
 * Netlify Scheduled Function — triggers the daily ops digest at 8am ET by
 * POSTing to the /api/cron/send-ops-digest route.
 *
 ...
 * surface failures without retrying (the next tick is 1 day away).
 */
...
export const handler = schedule("0 12 * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-send-ops-digest] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-send-ops-digest] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-send-ops-digest]", err);
    return {
      statusCode: 500,
