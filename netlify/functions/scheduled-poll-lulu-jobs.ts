/**
 * Netlify Scheduled Function — triggers the Lulu print-job status poll every
 * 30 minutes by POSTing to the /api/cron/poll-lulu-jobs route.
 *
 ...
 * surface failures without retrying (the next tick is 30 minutes away).
 */
import { schedule } from "@netlify/functions";

const ROUTE = "/api/cron/poll-lulu-jobs";

export const handler = schedule("13-43/30 * * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-poll-lulu-jobs] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-poll-lulu-jobs] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-poll-lulu-jobs]", err);
    return {
      statusCode: 500,
