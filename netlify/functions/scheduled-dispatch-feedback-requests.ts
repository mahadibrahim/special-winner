/**
 * Netlify Scheduled Function — triggers the post-event feedback dispatch
 * sweep every hour by POSTing to the /api/cron/dispatch-feedback-requests
 * route.
 *
 ...
 * surface failures without retrying (the next tick is 1 hour away).
 */
import { schedule } from "@netlify/functions";

const ROUTE = "/api/cron/dispatch-feedback-requests";

export const handler = schedule("7 * * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-dispatch-feedback-requests] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-dispatch-feedback-requests] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-dispatch-feedback-requests]", err);
    return {
      statusCode: 500,
