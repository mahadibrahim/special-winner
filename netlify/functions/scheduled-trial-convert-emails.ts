/**
 * Netlify Scheduled Function — triggers the trial-convert follow-up email
 * sweep every day at 4:47 AM UTC by POSTing to the
 * /api/cron/trial-convert-emails route.
 *
...
const ROUTE = "/api/cron/trial-convert-emails";

export const handler = schedule("47 4 * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-trial-convert-emails] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-trial-convert-emails] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-trial-convert-emails]", err);
    return {
      statusCode: 500,
