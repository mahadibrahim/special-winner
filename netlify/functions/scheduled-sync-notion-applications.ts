/**
 * Netlify Scheduled Function — triggers the Notion ATS retry sync every
 * hour by POSTing to the /api/cron/sync-notion-applications route.
 *
...
const ROUTE = "/api/cron/sync-notion-applications";
...
export const handler = schedule("0 * * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-sync-notion-applications] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-sync-notion-applications] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-sync-notion-applications]", err);
    return {
      statusCode: 500,
