/**
 * Netlify Scheduled Function — triggers the class-slot materialization +
 * auto-booking sweep every day at 4:23 AM UTC by POSTing to the
 * /api/cron/materialize-class-sessions route.
 *
...
const ROUTE = "/api/cron/materialize-class-sessions";

export const handler = schedule("23 4 * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-materialize-class-sessions] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-materialize-class-sessions] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-materialize-class-sessions]", err);
    return {
      statusCode: 500,
