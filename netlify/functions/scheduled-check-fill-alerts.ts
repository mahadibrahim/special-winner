/**
 * Netlify Scheduled Function — triggers the "needs players" fill-alert sweep
 * every 15 minutes by POSTing to the /api/cron/check-fill-alerts route.
 *
 ...
 * surface failures without retrying (the next tick is 15 minutes away).
 */
import { schedule } from "@netlify/functions";

const ROUTE = "/api/cron/check-fill-alerts";

export const handler = schedule("4-49/15 * * * *", async () => {
  ...
  if (!base) {
    console.error(
      "[scheduled-check-fill-alerts] no site URL in env (URL / PUBLIC_APP_URL)",
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[scheduled-check-fill-alerts] ${ROUTE} → ${res.status}: ${body}`,
      );
    ...
    console.error("[scheduled-check-fill-alerts]", err);
    return {
      statusCode: 500,
